...
Sebagian Fitur Masi Ada Yg Error Lagi Males Ngefix Bruh
Kalo Lu Bisa Ngefix Ya Fix Aja Bebas
Jan Lupa Subscribe DaffaUhuy & DappaUhuy
...
const LolHuman = '7201f836013adbe80a4d4182'
...
break
		...
		if (isBanned) return reply(ind.baned())
				...
				dappa.sendMessage(from, ttp, sticker, {quoted: mek})
				break
			case 'thunder':
                 if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
...
` 
		...
		reply(ind.wait)
				...
				if (isBanned) return reply(ind.baned())
                    ...
                    dappa.sendMessage(from, buffer, image, {caption: 'Nih kak.. *Jangan lupa subscribe DappaUhuy*', quoted: mek})
                    break
				case 'lirik':    
               			if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
